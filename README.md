...
一番手間なのがここですが、**最初に1回だけ**です。

1. [Google Cloud](https://console.cloud.google.com/) で **YouTube Data API v3 を有効化**
2. **OAuth クライアントの認証情報**を作り、リポジトリ直下に置く
3. アップ先の **YouTube チャンネルを作る**
4. 初回だけ **ブラウザで OAuth 認証** → トークンが保存される

2回目以降は保存トークンで自動。＝ **認証が終われば、もう手作業はほぼゼロ**です。

> 詳しい GCP の手順とコマンドは [`CLAUDE.md`](CLAUDE.md) を参照（Claude に「YouTube 認証をセットアップして」と頼んでも案内してくれます）。
